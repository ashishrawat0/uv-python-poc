...
def save_mock_data():
    ...
    for i in range(length):
        ...
        data = bytes(str(commission_type), "utf-8") + u2py.VM + bytes(str(commission_type), "utf-8") + u2py.VM + bytes(
            str(commission_type), "utf-8")
        ...
        data = bytes(str(item_no + random.randint(5000, 40000)), "utf-8") + u2py.VM + bytes(
            str(item_no + random.randint(5000, 40000)), "utf-8") + u2py.VM + bytes(
            str(item_no + random.randint(5000, 40000)), "utf-8")
        transaction_file.writev(transaction_id, 9, data)

        data = bytes(sv_changed_1, "utf-8") + u2py.VM + bytes(sv_changed_2, "utf-8") + u2py.VM + bytes(sv_changed_3,
                                                                                                       "utf-8")
        transaction_file.writev(transaction_id, 11, data)

        data1 = bytes(str(markdown_1), "utf-8") + u2py.VM + bytes(str(markdown_2), "utf-8") + u2py.VM + bytes(
            str(markdown_3), "utf-8")
        transaction_file.writev(transaction_id, 12, data1)

        data = bytes(str(mkup_strote_qty), "utf-8") + u2py.VM + bytes(str(mkup_strote_qty), "utf-8") + u2py.VM + bytes(
            str(mkup_strote_qty), "utf-8")
        transaction_file.writev(transaction_id, 13, data)

        data = bytes(str(description_1), "utf-8") + u2py.VM + bytes(str(description_2), "utf-8") + u2py.VM + bytes(
            str(description_3), "utf-8")
        transaction_file.writev(transaction_id, 14, data)

        data = bytes(str('CASH'), "utf-8") + u2py.VM + bytes(str('CASH'), "utf-8") + u2py.VM + bytes(str('CASH'),
                                                                                                     "utf-8")
        ...
        data = bytes(str(line_discount), "utf-8") + u2py.SM + bytes(str(line_discount), "utf-8") + u2py.VM + bytes(
            str(line_discount), "utf-8") + u2py.SM + bytes(str(line_discount), "utf-8") + u2py.VM + bytes(
            str(line_discount), "utf-8") + u2py.SM + bytes(str(line_discount), "utf-8")
        ...
        data = bytes(str(random.randint(50000, 70000)), "utf-8") + u2py.VM + bytes(str(random.randint(50000, 70000)),
                                                                                   "utf-8") + u2py.VM + bytes(
            str(random.randint(50000, 70000)), "utf-8")
        ...
        data = bytes(str(random.randint(10000, 30000)), "utf-8") + u2py.VM + bytes(str(random.randint(10000, 30000)),
                                                                                   "utf-8") + u2py.VM + bytes(
            str(random.randint(10000, 30000)), "utf-8")
        transaction_file.writev(transaction_id, 77, data)

        transaction_file.writev(transaction_id, 82, masked_acct_no)

        data = bytes(str(rental_id), "utf-8") + u2py.VM + bytes(str(rental_id), "utf-8") + u2py.VM + bytes(
            str(rental_id), "utf-8")
        transaction_file.writev(transaction_id, 90, data)

        data = bytes(ship_carrier_1, "utf-8") + u2py.VM + bytes(ship_carrier_2, "utf-8") + u2py.VM + bytes(
            ship_carrier_3, "utf-8")
        transaction_file.writev(transaction_id, 104, data)

        data = bytes(ship_track_no_1, "utf-8") + u2py.VM + bytes(ship_track_no_2, "utf-8") + u2py.VM + bytes(
            ship_track_no_3, "utf-8")
        transaction_file.writev(transaction_id, 105, data)

        data = bytes(str(random.randint(10, 300)), "utf-8") + u2py.VM + bytes(str(random.randint(10, 300)),
                                                                              "utf-8") + u2py.VM + bytes(
            str(random.randint(100, 300)), "utf-8")
        ...
        data = bytes(coupon_code_1, "utf-8") + u2py.VM + bytes(coupon_code_2, "utf-8") + u2py.VM + bytes(coupon_code_3,
                                                                                                         "utf-8")
        ...
        if discount_type != 'nan':
            data = bytes(str(discount_type), "utf-8") + u2py.VM + bytes(str(discount_type), "utf-8") + u2py.VM + bytes(
                str(discount_type), "utf-8")
            transaction_file.writev(transaction_id, 159, data)

        data = bytes(discount_pod_type_1, "utf-8") + u2py.VM + bytes(discount_pod_type_2, "utf-8") + u2py.VM + bytes(
            discount_pod_type_3, "utf-8")
        transaction_file.writev(transaction_id, 160, data)

        data = bytes(discount_pod_amount_1, "utf-8") + u2py.VM + bytes(discount_pod_amount_2,
                                                                       "utf-8") + u2py.VM + bytes(discount_pod_amount_3,
                                                                                                  "utf-8")
        transaction_file.writev(transaction_id, 161, data)

        transaction_file.writev(transaction_id, 162, discount_employee_id)

        data = bytes(discount_reason_code_1, "utf-8") + u2py.VM + bytes(discount_reason_code_2,
                                                                        "utf-8") + u2py.VM + bytes(
            discount_reason_code_3, "utf-8")
        transaction_file.writev(transaction_id, 163, data)

        data = bytes(promos_id_1, "utf-8") + u2py.VM + bytes(promos_id_2, "utf-8") + u2py.VM + bytes(promos_id_3,
                                                                                                     "utf-8")
        transaction_file.writev(transaction_id, 165, data)

        data = bytes(in_add_mrkdn_1, "utf-8") + u2py.VM + bytes(in_add_mrkdn_2, "utf-8") + u2py.VM + bytes(
            in_add_mrkdn_3, "utf-8")
        transaction_file.writev(transaction_id, 166, data)

        data = bytes(str(discount_conversion), "utf-8") + u2py.VM + bytes(str(discount_conversion),
                                                                          "utf-8") + u2py.VM + bytes(
            str(discount_conversion), "utf-8")
        ...
        data = bytes(str(random.randint(100000, 300000)), "utf-8") + u2py.VM + bytes(
            str(random.randint(100000, 300000)), "utf-8") + u2py.VM + bytes(str(random.randint(100000, 300000)),
                                                                            "utf-8")
        ...
        data = bytes(str(comm_sale_amount_1), "utf-8") + u2py.VM + bytes(str(comm_sale_amount_2),
                                                                         "utf-8") + u2py.VM + bytes(
            str(comm_sale_amount_3), "utf-8")
        transaction_file.writev(transaction_id, 241, data)

        data = bytes(str(comm_rate), "utf-8") + u2py.VM + bytes(str(comm_rate), "utf-8") + u2py.VM + bytes(
            str(comm_rate), "utf-8")
        transaction_file.writev(transaction_id, 242, data)

        if comm_empl_id != 'nan':
            comm_empl_id = int(float(comm_empl_id))
            data = bytes(str(comm_empl_id), "utf-8") + u2py.VM + bytes(str(comm_empl_id), "utf-8") + u2py.VM + bytes(
                str(comm_empl_id), "utf-8")
            transaction_file.writev(transaction_id, 244, data)

        data = bytes(str(comm_empl_percent_used), "utf-8") + u2py.VM + bytes(str(comm_empl_percent_used),
                                                                             "utf-8") + u2py.VM + bytes(
            str(comm_empl_percent_used), "utf-8")
        transaction_file.writev(transaction_id, 245, data)

        data = bytes(str(comm_empl_type), "utf-8") + u2py.VM + bytes(str(comm_empl_type), "utf-8") + u2py.VM + bytes(
            str(comm_empl_type), "utf-8")
        ...
        phone = phone.replace('-', '')
        employee_file.writev(operator_id, 1, first_name)

        employee_file.writev(operator_id, 2, last_name)

        employee_file.writev(operator_id, 17, first_name[0:3].upper() + str(random.randint(10, 100)))

        employee_file.writev(operator_id, 27, first_name)
